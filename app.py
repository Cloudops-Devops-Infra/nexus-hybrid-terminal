...
with st.sidebar:
    st.markdown('<div class="logo"><div class="logo-title">⚡ NEXUS HYBRID</div><div class="logo-sub">INFERENCE TERMINAL</div></div>', unsafe_allow_html=True)

    st.markdown('<span class="slabel">// backend runtime</span>', unsafe_allow_html=True)
    runtime_mode = st.selectbox("Runtime", ["Local (Ollama)", "Cloud (Groq API)"], label_visibility="collapsed")

    if runtime_mode == "Local (Ollama)":
        ...
        try:
            ...
            models = [m["id"] for m in r.json().get("data", [])] if status == "online" else []
        except Exception:
            status = "offline"
            models = []
        if status == "online" and not models:
            models = ["qwen2:0.5b"]
    else:
        BASE_URL = "https://api.groq.com/openai"
        st.markdown('<span class="slabel">// cloud credentials</span>', unsafe_allow_html=True)
        api_key = st.text_input("Groq API Key", type="password", value=os.environ.get("GROQ_API_KEY", ""), label_visibility="collapsed", placeholder="gsk_...")
    ...
    if status == "online":
        st.markdown('<div class="pill on"><div class="dot on"></div>ENGINE READY</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="pill off"><div class="dot off"></div>SETUP REQUIRED</div>', unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<span class="slabel">// active model</span>', unsafe_allow_html=True)
    ...
    if models:
        ...
        chosen_id = None
        if runtime_mode == "Local (Ollama)":
            st.markdown('<div class="minfo"><b>OLLAMA NOT FOUND</b><br>Start Ollama on your Windows PC.</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="minfo"><b>NO KEY PROVIDED</b><br>Input a valid Groq API Key.</div>', unsafe_allow_html=True)
...
st.markdown('<div class="psub">// HYBRID CORE INFERENCE TERMINAL</div>', unsafe_allow_html=True)
st.markdown(f'<div class="modelbar"><span class="tag">ACTIVE ROUTE ›</span><span class="name">{runtime_mode} : {chosen_id or "None"}</span></div>', unsafe_allow_html=True)
st.markdown("---")
...
if prompt := st.chat_input("Transmit message…"):
    ...
    if status != "online" or not chosen_id:
        st.warning("⚠ Engine offline. Verify your setup in the sidebar.")
    else:
        ...
        with st.chat_message("assistant", avatar="⚡"):
            ...
            try:
                res = requests.post(
                    CHAT_ENDPOINT,
                    headers=HEADERS,
                    json={"model": chosen_id, "messages": api_msgs, "temperature": temperature, "top_p": top_p, "stream": True},
                ...
                if res.status_code != 200:
                    try:
                        ...
                        err_msg = res.text
                    placeholder.error(f"❌ Cloud API Error ({res.status_code}): {err_msg}")
                    st.stop()
                
                for chunk in parse_stream(res):
                    full += chunk
